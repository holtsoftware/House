...
{
	public class BME280Sensor : ITempreatureHumidityPressureSensor
	{
		#region Registers
		//Register names:
		...
		{
			// Get the selector String for bus
			String busSelector = I2cDevice.GetDeviceSelector("I2C1");

			// Find the I2C bus controller with our selector string
			var devices = await DeviceInformation.FindAllAsync(busSelector);
			if(devices.Count == 0)
			{
		...
		public Task<double> GetTemperatureCelsiusAsync()
		{
			return Task.Run(() => readTemperatureCelsius());
		}
		...
		/// </summary>
		/// <returns></returns>
		public Task<double> GetTemperatureFahrenheitAsync()
		{
			return Task.Run(() => ConvertToFahrenheit(readTemperatureCelsius()));
		}
